...
import { createEvent, EVENT_TYPES, getEventById, updateEvent } from '../services/event-service.js';
...
eventRouter.patch('/profiles/:profileId/events/:eventId', asyncHandler(async (req, res) => {
  ...
  const event = await updateEvent({ eventId: req.params.eventId, profileId: req.params.profileId, body });
...
eventRouter.delete('/profiles/:profileId/events/:eventId', asyncHandler(async (req, res) => {
  await assertProfileAccess(req.auth.userId, req.params.profileId);
  const [result] = await db.execute('DELETE FROM health_events WHERE id = ? AND profile_id = ?', [req.params.eventId, req.params.profileId]);
  if (!result.affectedRows) throw new HttpError(404, 'Evento no encontrado.');
