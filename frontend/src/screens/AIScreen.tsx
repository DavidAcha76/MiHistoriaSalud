...
export function AIScreen({ navigation }: any) {
  ...
  return <Screen>
    ...
    {result ? <><SectionTitle>Resultado</SectionTitle><Card style={{ borderColor: colors.ai }}><Text style={{ fontSize: 12, fontWeight: '800', color: colors.ai }}>IA · {result.provider} / {result.model}</Text><Text style={{ marginTop: 10, color: colors.text, lineHeight: 22 }}>{result.summary}</Text></Card><SectionTitle>Datos incompletos</SectionTitle><Card>{result.incompleteData.length ? result.incompleteData.map((item, index) => <Text key={index} style={{ color: colors.text, marginBottom: 7 }}>• {item}</Text>) : <Muted>No se señalaron datos incompletos.</Muted>}</Card><SectionTitle>Posibles contradicciones</SectionTitle><Card>{result.contradictions.length ? result.contradictions.map((item, index) => <Text key={index} style={{ color: colors.text, marginBottom: 7 }}>• {item}</Text>) : <Muted>No se señalaron contradicciones.</Muted>}</Card><SectionTitle>Preguntas informativas</SectionTitle><Card>{result.questions.map((item, index) => <Text key={index} style={{ color: colors.text, marginBottom: 7 }}>• {item}</Text>)}</Card><Card style={{ backgroundColor: '#F6F2FF' }}><Text style={{ color: colors.ai, fontWeight: '800' }}>Importante</Text><Text style={{ color: colors.text, marginTop: 6, lineHeight: 20 }}>{result.disclaimer}</Text></Card></> : null}
